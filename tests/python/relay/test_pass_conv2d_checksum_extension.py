...
def test_single_non_unit_strided_standard_conv2d():
    """Simple testcase."""

    def before(x, w1):
        args = [x, w1]
        y = relay.nn.conv2d(x, w1, out_dtype="int32",strides=(2,2))
        return relay.Function(args, y)
    ...
    def expected(x, w1):
        ...
        y0 = relay.cast(x, dtype="int32")
        y1 = relay.sum(y0, axis=[0], keepdims=True)
        y2 = relay.cast(w1, dtype="int32")
        y3 = relay.nn.reduced_input(y1, strides=(2,2), weight_shape=[1, w1.type_annotation.shape[1], w1.type_annotation.shape[2], w1.type_annotation.shape[3]],
                                    kernel_layout="OIHW", data_layout="NCHW")
        y4 = relay.sum(y2, axis=[0], keepdims=True)
        y5 = relay.nn.conv2d(y3, y4, padding=(0,0), channels=1, groups=1, kernel_size=[w1.type_annotation.shape[2], w1.type_annotation.shape[3]], out_dtype="int64")
        y6 = relay.nn.conv2d(x, w1, padding=(0,0), out_dtype="int32",strides=(2,2))
        y7 = relay.cast(y6, dtype="int64")
        y8 = relay.sum(y5, axis=[0, 1, 2, 3])
        y9 = relay.sum(y7, axis=[0, 1, 2, 3])
        y10 = relay.not_equal(y8, y9)
...
#reduced input transforms input with redution to the filter dimensions (NCHW, OIHW)
    check((1, 4, 20, 20),(1, 4, 10, 10))
    check((1, 831, 16, 16),(1, 831, 6, 6))
    check((5, 20, 14, 14),(2, 20, 6, 6))
    check((5, 8, 10, 12),(14, 8, 4, 6))


def test_single_non_unit_asymmetric_strided_standard_conv2d():
    """Simple testcase."""

    def before(x, w1):
        args = [x, w1]
        y = relay.nn.conv2d(x, w1, out_dtype="int32",strides=(2,3))
        return relay.Function(args, y)
    ...
    def expected(x, w1):
        ...
        y1 = relay.sum(y0, axis=[0], keepdims=True)
        y2 = relay.cast(w1, dtype="int32")
        y3 = relay.nn.reduced_input(y1, strides=(2,3), weight_shape=[1, w1.type_annotation.shape[1], w1.type_annotation.shape[2], w1.type_annotation.shape[3]],
                                    kernel_layout="OIHW", data_layout="NCHW")
        y4 = relay.sum(y2, axis=[0], keepdims=True)
        y5 = relay.nn.conv2d(y3, y4, padding=(0,0), channels=1, groups=1, kernel_size=[w1.type_annotation.shape[2], w1.type_annotation.shape[3]], out_dtype="int64")
        y6 = relay.nn.conv2d(x, w1, padding=(0,0), out_dtype="int32",strides=(2,3))
        y7 = relay.cast(y6, dtype="int64")
        y8 = relay.sum(y5, axis=[0, 1, 2, 3])
        y9 = relay.sum(y7, axis=[0, 1, 2, 3])
        y10 = relay.not_equal(y8, y9)
...
#reduced input transforms input with redution to the filter dimensions
    check((1, 4, 20, 21),(1, 4, 10, 9))
    check((1, 831, 18, 18),(1, 831, 6, 6))
    check((5, 20, 14, 15),(2, 20, 6, 6))
    check((5, 8, 10, 12),(2, 8, 4, 6))
...
def test_single_non_unit_strided_standard_conv2d_nhwc():
    """Simple testcase."""

    def before(x, w1):
        args = [x, w1]
        y = relay.nn.conv2d(x, w1, out_dtype="int32",strides=(2,2),kernel_layout="HWIO", data_layout="NHWC")
        return relay.Function(args, y)
    ...
    def expected(x, w1):
        ...
        y1 = relay.sum(y0, axis=[0], keepdims=True)
        y2 = relay.cast(w1, dtype="int32")
        y3 = relay.nn.reduced_input(y1, strides=(2,2), weight_shape=[w1.type_annotation.shape[0], w1.type_annotation.shape[1], w1.type_annotation.shape[2], 1], 
                                    kernel_layout="HWIO", data_layout="NHWC")
        y4 = relay.sum(y2, axis=[3], keepdims=True)
        y5 = relay.nn.conv2d(y3, y4, padding=(0,0), channels=1, groups=1, kernel_size=[w1.type_annotation.shape[0], w1.type_annotation.shape[1]], out_dtype="int64", kernel_layout="HWIO", data_layout="NHWC")
        y6 = relay.nn.conv2d(x, w1, padding=(0,0), out_dtype="int32",strides=(2,2), kernel_layout="HWIO", data_layout="NHWC")
        y7 = relay.cast(y6, dtype="int64")
        y8 = relay.sum(y5, axis=[0, 1, 2, 3])
        y9 = relay.sum(y7, axis=[0, 1, 2, 3])
        y10 = relay.not_equal(y8, y9)
...
#reduced input transforms input with redution to the filter dimensions (NHWC, HWIO)
    check((1, 20, 20, 4),  (10, 10, 4, 1))
    check((1, 16, 16, 831),(6, 6, 831, 1))
    check((5, 14, 14, 20), (6, 6, 20, 2))
    check((5, 10, 12, 8),  (4, 6, 8, 14))


def test_single_non_unit_strided_depthwise_conv2d_nhwc():
    """Simple testcase."""

    def before(x, w1):
        args = [x, w1]
        y = relay.nn.conv2d(x, w1, out_dtype="int32",groups=int(w1.type_annotation.shape[3]),channels=w1.type_annotation.shape[3],data_layout="NHWC", kernel_layout="HWIO", strides=(2,2))
        return relay.Function(args, y)
    ...
    def expected(x, w1):
        ...
        y1 = relay.sum(y0, axis=[0], keepdims=True)
        y2 = relay.cast(w1, dtype="int32")
        y3 = relay.nn.reduced_input(y1, strides=(2,2), weight_shape=[w1.type_annotation.shape[0], w1.type_annotation.shape[1], w1.type_annotation.shape[3], 1],
                                    kernel_layout="HWIO", data_layout="NHWC")
        y2 = relay.sum(y2, axis=[2], keepdims=True)
        y2 = relay.transpose(y2, axes=[0,1,3,2])
        y5 = relay.nn.conv2d(y3, y2, padding=(0,0), channels=1, groups=1, kernel_size=[w1.type_annotation.shape[0], w1.type_annotation.shape[1]], out_dtype="int64", kernel_layout="HWIO", data_layout="NHWC")
        y6 = relay.nn.conv2d(x, w1, out_dtype="int32",groups=int(w1.type_annotation.shape[3]),channels=w1.type_annotation.shape[3],data_layout="NHWC", kernel_layout="HWIO", strides=(2,2))
        y7 = relay.cast(y6, dtype="int64")
        y8 = relay.sum(y5, axis=[0, 1, 2, 3])
        y9 = relay.sum(y7, axis=[0, 1, 2, 3])
        y10 = relay.not_equal(y8, y9)
...
# x1,w1 (NWHC),HWOI
    ...
    check((1, 56, 56, 64), ( 4,  4, 1, 64))
    check((1, 16, 16, 831),(16, 16, 1, 831))
    check((5, 17, 17, 20), (16, 16, 1, 20))
    check((5, 17, 17, 8),  (12, 11, 1, 8))
    check((1, 56, 56, 37), (11, 11, 1, 37))
...
def test_single_depthwise_conv2d_nhwc():
    ...
    def before(x, w1):
        args = [x, w1]
        y = relay.nn.conv2d(x, w1, out_dtype="int32",groups=int(w1.type_annotation.shape[3]),channels=w1.type_annotation.shape[3],data_layout="NHWC", kernel_layout="HWIO")
        return relay.Function(args, y)
    ...
    def expected(x, w1,ones_shape):
        ...
        y6 = relay.nn.conv2d(x, w1, out_dtype="int32",groups=int(w1.type_annotation.shape[3]),channels=w1.type_annotation.shape[3],data_layout="NHWC", kernel_layout="HWIO")
        y7 = relay.cast(y6, dtype="int64")
        y8 = relay.sum(y5, axis=[0, 1, 2, 3])
        y9 = relay.sum(y7, axis=[0, 1, 2, 3])
        y10 = relay.not_equal(y8, y9)
    ...
    def check(x_shape, w_shape, ones_shape):
        ...
        w1 = relay.var("w1", shape=w_shape, dtype="int8")
        y_before = before(x, w1)
        y_before = run_opt_pass(y_before, transform.InferType())
        y = run_opt_pass(y_before, transform.Extend2DConv())
        y = run_opt_pass(y, transform.InferType())
        print(y)
        y_expected = expected(x, w1, ones_shape)
...
# x1,w1,ones (NWHC),HWIO, HWIO
    check((1, 56, 56, 64), (3,   3, 1,  64), (54, 54, 1, 64))
    check((1, 16, 16, 831),(16, 16, 1, 831), (1,1,1,831))
    check((5, 17, 17, 20), (16, 16, 1,  20), (2,2,1,20))
    check((5, 17, 17, 8),  (12, 11, 1,   8), (6,7,1,8))
    check((1, 56, 56, 37), (11, 11, 1,  37), (46,46,1,37))
...
def verify_unit_strided(data_shape, data_dtype, kernel_shape, kernel_dtype, out_dtype):
    ...
    np.testing.assert_equal(golden_output.get_output(1).numpy(), np.int8(0))


def verify_non_unit_strided(data_shape, data_dtype, kernel_shape, kernel_dtype,out_dtype, stride):
    def get_inputs(data_shape, data_dtype, kernel_shape, kernel_dtype):
        # Keeping inputs multiple of 4 because of a bug in Average Pool2d
        # https://discuss.tvm.apache.org/t/pool2d-gives-bad-output-for-integer-inputs/3377
    ...
    def create_conv(data_shape, data_dtype, kernel_shape, kernel_dtype, stride):
        x  = relay.var("data", shape=data_shape, dtype=data_dtype)
        w1 = relay.var("kernel", shape=kernel_shape, dtype=kernel_dtype)
        args = [x,w1]
        y = relay.nn.conv2d(x, w1, strides=stride, padding=(0,0), out_dtype=out_dtype)
        return relay.Function(args, y)

    golden_inputs = get_inputs(data_shape, data_dtype, kernel_shape, kernel_dtype)
    ref_func = create_conv(data_shape, data_dtype, kernel_shape, kernel_dtype, stride)
    ref_func = run_opt_pass(ref_func, transform.Extend2DConv())
    ref_func = run_opt_pass(ref_func, transform.InferType())
    golden_output = get_output(ref_func, golden_inputs)
    # Test if checksum unequivalence is wrong = intended
...
def test_output_unit_strided():
    verify_unit_strided((1, 37, 56, 56),"int8",(1, 37, 11, 11),"int8", "int32")
    verify_unit_strided((2, 17, 20, 20),"int8",(1, 17, 11, 11),"int8", "int32")
    verify_unit_strided((3, 44, 123, 321),"int8",(3, 44, 123, 44),"int8", "int32")
    verify_unit_strided((5, 373, 20, 20),"int8",(12, 373, 11, 11),"int8", "int32")
    verify_unit_strided((1, 23, 201, 20),"int8",(1, 23, 11, 11),"int8", "int32")
    verify_unit_strided((83, 69, 12, 12),"uint8",(1, 69, 11, 11),"uint8", "uint32")
    verify_unit_strided((5, 373, 20, 20),"uint8",(12, 373, 11, 11),"uint8", "uint32")
    verify_unit_strided((132, 16, 23, 132),"uint8",(1, 16, 11, 11),"uint8", "uint32")
    verify_unit_strided((5, 66, 123, 321),"uint8",(12, 66, 11, 11),"uint8", "uint32")

def test_output_non_unit_strided():
    verify_non_unit_strided((1,  2, 20, 30),"int8",(1, 2, 4, 4),"int8", "int32", (2,2))
    verify_non_unit_strided((1,  1,  9, 12),"int8",(1, 1, 3, 3),"int8", "int32", (3,3))
    verify_non_unit_strided((1,  1,  12, 12),"int8",(1, 1, 6, 6),"int8", "int32", (6,6))
    verify_non_unit_strided((12, 2,  6,  6),"int8",(1, 2, 2, 3),"int8", "int32", (2,3))
    verify_non_unit_strided((12, 2,  6,  6),"int8",(1, 2, 2, 2),"int8", "int32", (2,2))
    verify_non_unit_strided((1,  1,  6,  9),"int8",(2, 1, 3, 3),"int8", "int32", (3,3))

if __name__ == "__main__":
    test_single_standard_conv2d()
    test_single_non_unit_strided_standard_conv2d()
    test_single_depthwise_conv2d()
    test_single_non_unit_strided_depthwise_conv2d_nhwc()
    test_single_non_unit_strided_standard_conv2d_nhwc()
    test_multiple_standard_conv2d()
    test_mixed_conv2d()
    test_single_non_unit_asymmetric_strided_standard_conv2d()
    test_output_unit_strided()
    test_output_non_unit_strided()