...
# Derived ExprVisitor with additional Dictionaries Expr<->BUF
class GraphAnalyzer(ExprVisitor):
    ...
    def run(self, func):
        """ Fetch BufferInfos in {Expr : Bufs} with DFS order """
    ...
    def createBufInfos(self, expr, static):
        ...
        def makeBuf(t):
            ...
            bufInfo.size = RelayType(t).getSize()
    ...
    def makeNet(self):
        """create network (self defined!) graph from BufferInfos to feed into networkx DiGraph"""
        n = Network()
        ...
        for expr, bufsInfo in self.exprToBufInfos.items():
            if isinstance(expr, relay.Call):
                ...
                op = n.addOp(self.exprToLabel[expr], inBufs, exprToBufs[expr])
        self.OpToExpr = {v: k for k, v in self.exprToOp.items()}
