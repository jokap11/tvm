from .exec_timeout import exec_timeout
from .ilpsolver import ILPSolver
from .schedule_sp_dec import decompose_sp_graph, SPType, EfficientSubgraph
from .schedule_sp import sp_schedule
from .schedule_heuristic import schedule_heuristic
...
def is_sp_graph_impl(esg):
    ...
    if len(G.nodes) < 3:
        return True
    spType, esg1, esg2 = decompose_sp_graph(esg)
    if spType == SPType.INVALID:
        return False
    G1 = esg1.view()
    G2 = esg2.view()
    isSP = True
...
def is_sp_graph(G):
    return is_sp_graph_impl(EfficientSubgraph(G))
...
class SchedOpt:
    ...
    def solve(self, G):
        ...
        # Intermediate as a TUPLE might be G.out_degree()!=0
        ...
        # Priority achieved with this
        G.nodes[sourceNode]["outsize"] = 1
        G.nodes[sinkNode]["outsize"] = 1

        sched = self.solveSubproblem(EfficientSubgraph(G))
    ...
    def solveWithTimeout(self, G, solveFunc, timeoutFunc, timeout=1.5):
        ...
        try:
            ids = exec_timeout(timeout, inExternalProcess)
