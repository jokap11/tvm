...
class Lifetimes:
    """Dictionary for LifeTime ranges + set of vars alive at time t"""
    def __init__(self):
...
class MemoryPlanner:
    """ Individual problem to allocate buffer into Arena (after scheduling)"""
    ...
    def createOptimalLayout(self):
        ...
        offsets = solve(bufSizes, conflicts)
...
def memLayoutWithTimeout(n, planner, func, timeout=0.5, noILP=False):
    ...
    if not noILP:
        ...
        try:
            idsToData = exec_timeout(timeout, inExternalProcess)
