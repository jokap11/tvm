...
class SplitConfig:
    ...
    def inferUp(self):
        # print("inferUp", exprToStr(self.expr))
        if self.splitType == SplitType.NOTSPLIT:
            inShape = getShape(self.expr.args[0])
        ...
        if opArgTy in [OpArgType.TRIVIAL, OpArgType.SIMPLE]:
            ...
            if name == "nn.max_pool2d" or name == "nn.avg_pool2d":
                ...
                inShape = getShape(self.expr.args[1])
                wShape = getShape(self.expr.args[0])
            ...
            wShape = getShape(self.expr.args[1])
            numBatches = wShape[0]
    ...
    def inferDown(self):
        # print("inferDown", exprToStr(self.expr))
        opArgTy = getOpArgType(self.expr)
        name = self.expr.op.name
        if opArgTy in [OpArgType.TRIVIAL, OpArgType.SIMPLE]:
            # handle special cases: pooling, padding
    ...
    def inferFTP(self, isUpwards):
        ...
        padding = normalizePadding(self.expr.attrs["padding"])
        padding = {
            widthAx: (getNormalizedPaddingPair(padding, True)),
            heightAx: (getNormalizedPaddingPair(padding, False)),
        }
    ...
    def getInSize(self):
        return getSize(getCallInput(self.expr))

    def getOutSize(self):
        return getSize(self.expr)

    def getOpSize(self):
        return self.getInSize() + self.getOutSize()

    def getInPartSize(self):
        return getTypeSize(getCallInput(self.expr)) * self.inSplit.getMaxNumPartElements()

    def getOutPartSize(self):
        return getTypeSize(self.expr) * self.outSplit.getMaxNumPartElements()

    def getInPartSize_fix(self, partNum):
        size = 1
        for dim in self.inSplit.getSplitShape(partNum):
            size *= dim
        return size * getTypeSize(getCallInput(self.expr))

    def getOutPartSize_fix(self, partNum):
        size = 1
        for dim in self.outSplit.getSplitShape(partNum):
            size *= dim
        return size * getTypeSize(self.expr)
...
class SplitPath:
            ...
            if wasAdjusted:
                for i in range(len(self) - 2, -1, -1):
                    ...
                    if status == InferStatus.INVALID:
                        ...
                        print("while inferring up:", exprToStr(self[i].expr), self)
            ...
            if wasAdjusted:
                for i in range(1, len(self)):
                    ...
                    if status == InferStatus.INVALID:
                        ...
                        print("while inferring down:", exprToStr(self[i].expr), self)
                        raise RuntimeError("did not expect further adjustments")
            # print("~~~ done ~~~")
    ...
    def limitToMinSize(self):
        ...
        for i, cfg in enumerate(reversed(self.postCfgs)):
            if hasOverlappingInput(cfg.expr):
...
def createChainedSplitConfigs(expr, path, isUpwards):
    ...
    if fromCfg.splitType == SplitType.FTP and hasOverlappingInput(expr):
...
class PathDiscovery:
    ...
    def evaluateSize(self, mod):
        ...
        sz = memLayoutWithTimeout(testNet, planner, cbLayout)
        return sz.popitem()[1]
